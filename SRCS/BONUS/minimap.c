...
/*   Updated: 2025/02/20 19:24:21 by annabrag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	clear_window_2d(t_mlx *mlx)
{
	...
	if (mlx == NULL|| mlx->img_buff == NULL)
		return ;
	pixel.y = 0;
	while (pixel.y < (W_HEIGHT / 2))
	{
		pixel.x = 0;
		while (pixel.x < (W_WIDTH / 2))
		{
...
static void	__draw_tile(t_data *data, t_point tile)
{
	...
	pixel.x = tile.x * (TILE_SIZE / 2);
	pixel.y = tile.y * (TILE_SIZE / 2);
	x_end = pixel.x + (TILE_SIZE / 2); // NOT OK
	y_end = pixel.y + (TILE_SIZE / 2); //
	while (pixel.y < y_end)
	{
		pixel.x = tile.x * (TILE_SIZE / 2);
		while (pixel.x < x_end)
