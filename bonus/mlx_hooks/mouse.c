...
/*   Updated: 2025/03/04 18:21:24 by annabrag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	__handle_mouse(int x, int y, t_data *data)
{
	...
	game_s()->ray->player_rad = get_radian(data->player_dir);
	return (SUCCESS);
}

static int	__handle_title_screen_mouse(int button, int x, int y, t_game *game)
{
	t_layer	start_button;

	start_button = game->title_screen.start_button;
...
void	set_mouse_hooks(t_mlx *mlx, t_data *data, t_game *game)
{
	mlx_hook(mlx->win_ptr, MotionNotify, PointerMotionMask, &__handle_mouse,
			data);
	mlx_mouse_hook(mlx->win_ptr, &__handle_title_screen_mouse, game);
}
